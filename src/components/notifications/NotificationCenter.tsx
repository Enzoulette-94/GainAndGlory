  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="absolute right-0 top-full mt-2 w-80 sm:w-96 bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl z-20 overflow-hidden"
      >
        {/* Header */}
        ...
        <div className="max-h-96 overflow-y-auto">
          {notifications.length === 0 ? (
            ...
            notifications.map(notif => (
              <div
                key={notif.id}
                className={`flex items-start gap-3 px-4 py-3 hover:bg-slate-800/50 transition-colors border-b border-slate-700/30 last:border-0 ${!notif.read ? 'bg-red-900/10' : ''}`}
              >
                <span className="text-lg flex-shrink-0 mt-0.5">
                  {notifIcons[notif.type as NotificationType] ?? '🔔'}
                </span>
                <div className="flex-1 min-w-0">
                  <p className={`text-sm ${notif.read ? 'text-slate-300' : 'text-slate-100 font-medium'}`}>
