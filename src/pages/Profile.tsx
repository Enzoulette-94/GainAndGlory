...
export function ProfilePage() {
  ...
  const [recCategory, setRecCategory] = useState<'musculation' | 'course' | 'calisthenics' | 'crossfit'>('musculation');
  ...
  function openAddRecord(cat: 'musculation' | 'course' | 'calisthenics' | 'crossfit' = 'musculation') {
      ...
      >
        ...
        {dataLoading ? (
          ...
          <div className="space-y-4">
            ...
            {/* Crossfit */}
            {records.filter(r => r.category === 'crossfit').length > 0 && (
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <Flame className="w-3.5 h-3.5 text-orange-400" />
                  <span className="text-xs font-semibold text-[#a3a3a3] uppercase tracking-wider">Crossfit</span>
                  <button onClick={() => openAddRecord('crossfit')} className="ml-auto text-[10px] text-orange-400/50 hover:text-orange-400 transition-colors flex items-center gap-0.5">
                    <Plus className="w-3 h-3" />Ajouter
                  </button>
                </div>
                <div className="space-y-1.5">
                  {records.filter(r => r.category === 'crossfit').map((r, i) => (
                    <motion.div key={r.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.27 + i * 0.04 }}
                      className="flex items-center gap-2 px-3 py-2.5 bg-[#111111] border border-white/5 border-l-2 border-l-orange-800/50"
                    >
                      <Flame className="w-3.5 h-3.5 text-orange-400 flex-shrink-0" />
                      <span className="text-sm font-medium text-[#d4d4d4]">{r.title}</span>
                      <span className="text-[#6b6b6b] text-xs">•</span>
                      <span className="text-sm font-bold text-orange-400">{r.value}</span>
                      <span className="text-xs font-normal text-[#6b6b6b] ml-0.5">{r.unit}</span>
                      <div className="flex items-center gap-1 ml-auto flex-shrink-0">
                        <button onClick={() => openEditRecord(r)} className="p-1 text-[#6b6b6b] hover:text-[#d4d4d4] transition-colors" aria-label="Modifier"><Pencil className="w-3.5 h-3.5" /></button>
                        <button onClick={() => handleDeleteRecord(r.id)} className="p-1 text-[#6b6b6b] hover:text-red-400 transition-colors" aria-label="Supprimer"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
      ...
      >
        <div className="p-5 space-y-4">
          ...
          <div className="flex rounded overflow-hidden border border-white/10">
            ...
            <button
              type="button"
              onClick={() => { setRecCategory('crossfit'); setRecUnit('kg'); setRecError(''); }}
              className={`flex-1 flex items-center justify-center gap-1.5 py-2 text-xs font-medium border-l border-white/10 transition-colors ${
                recCategory === 'crossfit' ? 'bg-orange-500/15 text-orange-400' : 'text-[#6b6b6b] hover:text-[#d4d4d4]'
              }`}
            >
          ...
          {recCategory === 'musculation' ? (
            <>
              <div>
                ...
                {recTitle && (
                  <p className="text-sm font-semibold text-[#c9a870] mb-2">{recTitle}</p>
                )}
                <MusculationPickerContent
                  selected={recTitle}
                  onSelect={name => { setRecTitle(name); setRecError(''); }}
                />
              </div>
            ...
            <>
              <RunningRacePicker
                value={recTitle}
                onChange={(title, km) => { setRecTitle(title); setRecDistance(String(km)); setRecError(''); }}
              />
              <Input
                label="Durée"
                value={recValue}
          ...
          ) : recCategory === 'calisthenics' ? (
            <>
              <div>
                <p className="text-xs text-[#a3a3a3] uppercase tracking-wide font-medium mb-2">Exercice</p>
                {recTitle && (
                  <p className="text-sm font-semibold text-violet-400 mb-2">{recTitle}</p>
                )}
            ...
            <>
              <div>
                <p className="text-xs text-[#a3a3a3] uppercase tracking-wide font-medium mb-2">Exercice</p>
                {recTitle && (
                  <p className="text-sm font-semibold text-orange-400 mb-2">{recTitle}</p>
                )}
                  ...
                  <div className="flex rounded overflow-hidden border border-white/10 h-10">
                    {(['kg', 'reps'] as const).map((u, idx) => (
                      <button
                        ...
                        onClick={() => setRecUnit(u)}
                        className={`flex-1 text-sm font-medium transition-colors ${idx > 0 ? 'border-l border-white/10' : ''} ${
                          recUnit === u ? 'bg-orange-500/15 text-orange-400' : 'text-[#6b6b6b] hover:text-[#d4d4d4]'
                        }`}
                      >
                        {u}
                      </button>
