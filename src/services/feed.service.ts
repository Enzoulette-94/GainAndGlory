...
export const feedService = {
  ...
  async publishWorkout(userId: string, tonnage: number, setsCount: number, feedback?: string) {
    try {
      await db.from('activity_feed').insert({
        user_id: userId,
        type: 'workout',
        content: { type: 'workout', tonnage, sets_count: setsCount, feedback },
      });
    } catch { /* ignore */ }
  },

  // Publier une course dans le feed
  async publishRun(userId: string, distance: number, duration: number, pace: number, runType?: string) {
    try {
      await db.from('activity_feed').insert({
        user_id: userId,
        type: 'run',
        content: { type: 'run', distance, duration, pace, run_type: runType },
      });
    } catch { /* ignore */ }
  },
};
